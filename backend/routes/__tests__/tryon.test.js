...
describe('POST /tryon — WaveSpeed path', () => {
  beforeEach(() => { process.env.WAVESPEED_API_KEY = 'test-key'; });

  it('returns the generated image on success and logs the attempt against a valid productId', async () => {
  ...
  it('returns 422 and logs a failed attempt when generation fails', async () => {
  ...
  it('returns 500 (or 429 for a rate-limit message) and logs a failed attempt when generation throws', async () => {
...
describe('POST /tryon — Replicate path (no WAVESPEED_API_KEY)', () => {
  ...
  it('fetches the product image and calls the Replicate service', async () => {
    ...
    const res = await request(app)
      .post('/api/tryon')
      .field('productImageUrl', 'https://example.com/p.jpg')
      .field('productName', uniqueName('replicate-path'))
      .attach('userImage', png, 'me.png');
