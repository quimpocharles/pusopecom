...
**Implemented (2026-08-02): Done.** Every rounded/pill/shadow treatment described above is gone: the pill-shaped partner chips, the segmented pill category tabs (now flat underline tabs, matching the Component Specification's Tabs rule directly), the circular scroll/CTA buttons, and every `rounded-2xl`/`rounded-3xl` image frame all moved to sharp bordered surfaces. The dead Instafeed block is deleted, as recommended. The section rhythm changed more than this entry anticipated: the original try-on-led Hero is retired outright (it read as generic, the same register as most other pages) and the former "Latest Collection" section is promoted into the page's opening statement instead — full viewport height, its own image/text composition, not a full-bleed hero. A new Featured Team section was added before Our Partners (a monthly-rotating Organization spotlight, background set to the featured institution's own color — the one deliberate use of DESIGN_TOKENS.md's `institution.identity` slot). The Newsletter section was removed (it had no `onSubmit` handler or backend route — decorative only). Color values are now standardized too: every `color`/`background`/`border` that maps to a real token uses it (`bg-ink-900`, `text-white/NN` opacity utilities in place of hand-typed `rgba(255,255,255,X)` strings, `border-white/NN` in place of ad hoc border colors). Two things deliberately stayed literal, not tokenized: the alternating `#1a1a1a` section background (a second dark neutral this page uses for rhythm that has no defined token — collapsing it into `ink.900` would have erased the alternation, a visual change nobody asked for) now lives as a plain `bg-[#1a1a1a]` arbitrary class; and the decorative grid/stripe/grain texture gradients, which aren't semantic colors in the first place.
...
4. **Migrate `Home.jsx` and delete its dead Instafeed block** — Part 3. The platform's highest-traffic page, the one that's already closest in spirit to the target direction, and the clearest possible proof that the new system works in production, not just in documentation.
   **Status (2026-08-02): Done.** Geometry, dead-code removal, section-level rework, and color-token migration are all done — see the Part 3 entry above for the fuller account, including the Hero retirement and new Featured Team section neither this plan nor the original brief anticipated.
