...
4. **Migrate `Home.jsx` and delete its dead Instafeed block** — Part 3. The platform's highest-traffic page, the one that's already closest in spirit to the target direction, and the clearest possible proof that the new system works in production, not just in documentation.
   **Status (2026-08-02): Partially done.** Geometry, dead-code removal, and section-level rework are done — see the Part 3 entry above for the fuller account, including the Hero retirement and new Featured Team section neither this plan nor the original brief anticipated. Color-token migration (hex/`rgba()` → `ink`/`paper`) is still outstanding.
